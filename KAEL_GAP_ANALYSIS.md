...
| Low | Glyph/shadow atlas never evicts | GPU memory climbs for hours | **Done on Metal + Blade, app-facing.** Frame-protected LRU byte-budget eviction wired through the whole stack — `App::set_atlas_byte_budget` → `Window` → `PlatformWindow` → renderer → atlas (per-tile `last_used`, etagere region reclaim, end-of-frame evict with in-flight-window protection). **Metal**: on-device-verified (6 atlas tests). **Blade**: same code mirrored + render-loop wired, compile-verified via `--features macos-blade`. Default `None` = unchanged. **Remaining:** DirectX mirror (no Windows here). | XS (Metal+Blade done; DirectX pending) |
