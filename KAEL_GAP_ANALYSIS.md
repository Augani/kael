...
| Medium | Clip-path/mask | ~~Only rounded rects~~ | **Bridged & golden-verified, app-facing.** `ClipShape{Circle,Ellipse,ConvexPolygon}` + `contains()`/`coverage()`/`bounding_box()`/`rasterize_mask()`/`apply_clip_mask_bgra()`/`as_rounded_clip()`/`translate()`, `Window::with_clip_path()`, and the **`clip_path(shape, child)` wrapper element** (mirrors `effect_layer`/`image_cache`, element-relative coords). **Circles clip LIVE through the existing shader** (golden: center opaque, 4 corners cut), **polygons clip correctly via the mask** (golden triangle); 20 clip tests. **Remaining:** fuse the *polygon* mask sample into the live in-pass shader (polygons bbox-fallback in `with_clip_path` today). | M (circle live + app API; polygon in-pass fusion pending) |
