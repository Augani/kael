...
| Blend-mode dst-read, clip-path/mask, wide-gamut, gradient text, dual-Kawase blur | **Re-classified:** the "not pixel-verifiable on this machine" premise was wrong — the golden-image harness exists in-repo and the Metal backend is live here (see shipped list). These are now scoped, **verifiable** shader tasks (write the feature, add a golden test), not externally-gated ones — the multi-stop gradient expansion below is the worked proof. | None external — scoped renderer work; each needs its own golden test. |
