...
| Damage/dirty-region rendering, incremental layout, code hot-reload | XL multi-week rewrites. **Partial coverage already exists:** view-level dirty tracking (`dirty`/`dirty_views` gate re-render) and a now **content-aware frame checksum** (`Scene::structural_checksum` hashes quad backgrounds + glyph colors, not just geometry — the frame-identity primitive a safe skip-render needs, unit-tested). The XL remainder is fine-grained dirty-*region* (per-rect GPU updates), full incremental relayout, and code (not shader — `runtime_shaders` already hot-reloads shaders) hot-reload. | Dedicated multi-week workstreams. |
