...
| Blend-mode dst-read (Overlay/SoftLight/Difference only), clip-path/mask, wide-gamut, dual-Kawase blur | **Multiply + Screen are now bridged** (Metal, pixel-verified — see shipped list). The remaining three modes (Overlay/SoftLight/Difference) aren't expressible as fixed-function blend factors, so they need framebuffer-fetch `[[color(0)]]` — which on the MSAA quad pipeline forces per-sample shading unless given a dedicated pipeline; doable but heavier. Clip-path/mask needs a stencil/SDF mask path; wide-gamut isn't verifiable via the 8-bit readback; dual-Kawase only optimizes the working Gaussian. (`gradient text` was on this list by mistake — it already ships.) | None external — scoped renderer work; each needs its own golden test. |
