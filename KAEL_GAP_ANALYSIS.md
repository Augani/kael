...
- **Blend modes read the backdrop — Multiply + Screen (the "blend modes never sample backdrop" correctness bug, BRIDGED + pixel-verified on Metal):** these two (the most-used modes) are now real `src×dst` / screen blends evaluated by **dedicated fixed-function blend pipelines** (`DestinationColor`/`Zero` and `One`/`OneMinusSourceColor`), routed via a per-blend-mode **run-split** in `draw_quads` (the common all-normal run draws in one call, unchanged). The shader's old `src×src` self-blend for these modes is gone (it passes them through to the pipeline). Golden test: white-Multiply over a gray backdrop leaves the backdrop unchanged (real dst read), where the old self-blend turned it white; black-Screen likewise. No framebuffer fetch / MSAA cost / portability risk, and **no regression** — every existing solid/gradient golden test still passes through the new run-split. Metal-only for now (blade/directx keep their current approximation; no regression there).
...
| Blend-mode dst-read (Overlay/SoftLight/Difference only), clip-path/mask, wide-gamut, dual-Kawase blur | **Multiply + Screen are bridged** (Metal, pixel-verified — see shipped list). The remaining three modes (Overlay/SoftLight/Difference) aren't expressible as fixed-function blend factors, so they need framebuffer-fetch `[[color(0)]]` — and **this was attempted and is hard-blocked at build time**: the macOS Metal compiler rejects a `[[color(0)]]` fragment input (`"invalid 'color' attribute for input declaration"`) because the metallib must run on Intel/AMD Macs that lack programmable blending. A runtime `Option` fallback can't help — it fails to *compile*. Real options: an Apple-Silicon-only shader variant, or a separate compositing pass (multi-pass, heavier). Clip-path/mask needs a stencil/SDF mask path; wide-gamut isn't verifiable via the 8-bit readback; dual-Kawase only optimizes the working Gaussian. (`gradient text` was on this list by mistake — it already ships.) | Apple-GPU-targeted shader feature set, or a multi-pass compositor. |
