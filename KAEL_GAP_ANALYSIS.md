...
| Medium | Clip-path/mask | ~~Only rounded rects~~ | **Largely bridged & golden-verified.** `ClipShape{Circle,Ellipse,ConvexPolygon}` + `contains()`/`coverage()`/`bounding_box()`/`rasterize_mask()`/`apply_clip_mask_bgra()`/`as_rounded_clip()` + `Window::with_clip_path()`. **Circles clip LIVE through the existing shader** (`circle_clip_shape_renders_through_the_rounded_clip_shader`: center opaque, all 4 corners cut) and **arbitrary polygons clip correctly via the mask** (`arbitrary_triangle_clip_produces_correct_pixels`); 18 clip tests. **Remaining:** fuse the *polygon* mask sample into the live in-pass shader (polygons currently bbox-fallback in `with_clip_path`) + a `Styled::clip_path()` sugar. | M (circle live; polygon mask verified, in-pass fusion pending) |
