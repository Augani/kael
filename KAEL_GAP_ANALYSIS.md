...
| High | Clip-path/mask | Only rounded rects; samey clipped surfaces | **Working + golden-verified** — `ClipShape{Circle,Ellipse,ConvexPolygon}` with `contains()` (hit-test), `signed_distance()`/`coverage()` (SDF), `bounding_box()`, `rasterize_mask()` + `apply_clip_mask_bgra()`. **A real GPU-rendered quad clipped to a triangle produces correct pixels** (`arbitrary_triangle_clip_produces_correct_pixels`: interior opaque, exterior transparent); 11 tests total. **Remaining:** fuse the mask into the live shader pipeline (currently a post-render mask apply) + element-level `clip_path()` API. | XL (mechanism done & verified) |
