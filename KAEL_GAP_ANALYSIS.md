...
| Multi-stop gradients (>4 stops) | **No longer "unverifiable" — now just large.** The Rust builders resample >4 stops to 4 preserving endpoints (landed); the existing 4-stop pipeline is now pixel-verified (linear/radial/conic golden tests pass). Going beyond 4 is a real but **invasive** refactor: stop colors flow through `QuadVarying`/`PathVarying` `color0..color3` varyings across 3 render paths (quad/border/path), so >4 means either more varyings or moving color prep fragment-side. **Tractable and verifiable now** (a >4-stop golden test would confirm it) — just hot-path surgery deferred for stability, not gated. | None external — scoped renderer work, now verifiable in-repo. |
| Blend-mode dst-read, clip-path/mask, wide-gamut, gradient text, dual-Kawase blur | **Re-classified:** the "not pixel-verifiable on this machine" premise was wrong — the golden-image harness exists in-repo and the Metal backend is live here (see shipped list). These are now scoped, **verifiable** shader tasks (write the feature, add a golden test), not externally-gated ones. | None external — scoped renderer work; each needs its own golden test. |
