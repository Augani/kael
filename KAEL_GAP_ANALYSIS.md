...
| High | Glyph/shadow atlas never evicts | GPU memory climbs for hours | **Increment 1 landed: the safety-critical policy.** `select_atlas_evictions(tiles, total, max_bytes, current_frame)` — LRU byte-budget eviction that **never evicts a tile used in the current frame** (in-flight-render corruption guard), returning what it can safely shed; 4 unit tests incl. the frame-protection cases. **Remaining:** per-backend wiring (`last_used` tracking + calling it at end-of-frame + etagere `deallocate` on the selected tiles), verified against the real glyph lifecycle on-device. `TileId↔etagere::AllocId` round-trip already exists. | XL (policy done; per-backend wiring pending) |
