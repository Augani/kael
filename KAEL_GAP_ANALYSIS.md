...
| Blend-mode dst-read, clip-path/mask, wide-gamut, dual-Kawase blur | **Re-classified:** the "not pixel-verifiable on this machine" premise was wrong — the golden-image harness exists in-repo and the Metal backend is live here (see shipped list). These are now scoped, **verifiable** shader tasks (write the feature, add a golden test), not externally-gated. Measured per-item scope: blend-mode dst-read needs a pipeline restructure (quad pipeline uses fixed-function blending + MSAA, so reading the destination means doing all alpha-over in-shader); the others are localized. (`gradient text` was on this list by mistake — it already ships, see below.) | None external — scoped renderer work; each needs its own golden test. |
