...
|---|---|---|---|---|
| Critical | Damage/dirty-region present | Full clear + full re-encode every vsync on static screens | **Whole-frame checksum early-out LANDED (opt-in). Per-rect computation (`Scene::damage_since`) + GPU scissor+Load mechanism both IMPLEMENTED + golden-verified; remaining: live present-loop wiring** | XL (mostly done) |
...
| Medium | Glyph/shadow atlas never evicts | GPU memory climbs for hours | **Policy + Metal backend DONE & on-device-verified.** `select_atlas_evictions` (LRU byte-budget, never evicts a current-frame tile) is now WIRED into `MetalAtlas`: per-tile `last_used` frame stamping, `advance_frame()`, `evict_to_budget(max_bytes)` that reclaims each evicted tile's etagere region (`allocator.deallocate`) + ref-count + frees empty textures. On-device test (`evicts_lru_tiles_to_budget_and_protects_the_current_frame`): 4 tiles → 1-tile budget sheds the 3 LRU, protects the current-frame tile (stays cached), evicted tiles re-rasterize reusing reclaimed space. 5 atlas tests. **Remaining:** call `advance_frame`/`evict_to_budget` from the render loop driven by `GpuMemoryBudget`; replicate on blade/directx (compile-only here). | M (metal done+verified; render-loop call + other backends pending) |
| High | Canvas path deep-clone | Per-frame heap churn | Arc paths, interned state, retained Vec | M |
| High | Orphaned cache crates | ~~Unbounded TileCache~~ | **Largely addressed** — `TileCache::with_max_bytes` already enforces a byte budget with LRU eviction (`evict_to_budget`, unit-tested `tile_cache_evicts_lru_over_budget`); it's a public opt-in utility (like the new `LruImageCache`). Residual: no internal default consumer wires it (apps opt in) + `kael_cache::MemoryCache`/`CacheManager` remain available-but-app-driven. | S (mostly done) |
...
| Low | Clip-path/mask | ~~Only rounded rects~~ | **Bridged, app-facing, GPU-accelerated, golden-verified.** `ClipShape{Circle,Ellipse,ConvexPolygon}` + full geometry API, `Window::with_clip_path()`, the **`clip_path(shape, child)` element**, **circles clip LIVE through the existing shader** (golden), and **polygons clip on the GPU** via a compute-kernel mask multiply that golden-matches the CPU reference within 1 byte (`gpu_clip_mask_matches_the_cpu_reference_and_clips_a_triangle`); 21 clip tests. **Remaining (optimization only):** fuse the polygon mask sample *in-pass* into the quad fragment shader to drop the offscreen+composite — the result is already correct & GPU-accelerated. | Done; in-pass fusion is a perf refinement |
...
- **Canvas / one-Context2D (priority #3):** flat `save`/`restore` state stack + `translate`/`rotate`/`scale`/`set_global_alpha`/`clip_rect`; `draw_image`; `fill_ellipse`/`stroke_ellipse`; `Path::contains` (isPointInPath, lyon hit-test); `draw_text_aligned` (textAlign); `stroke_rect`/`stroke_rounded_rect`; and the remaining Canvas2D **path-building** ops on `Path` — `cubic_to` (`bezierCurveTo`), `arc`, `arc_to` (`arcTo`, the rounded-corner primitive), and `close_path` (`closePath`) — flattened to the existing GPU winding-fill so they need no shader work (endpoint geometry unit-tested, incl. exact `arcTo` tangent points). plus `ellipse` (web `ellipse()` — rotated radii + arc range, exact rotation geometry unit-tested), `rect` (`rect()`), and `round_rect` (`roundRect()`, single-radius, 4 corner arcs). `Path` now covers the full web path command set (`moveTo`/`lineTo`/`quadraticCurveTo`/`bezierCurveTo`/`arc`/`arcTo`/`ellipse`/`rect`/`roundRect`/`closePath`). Stroke styling gained fluent builders — `Stroke::cap` (`lineCap`), `join` (`lineJoin`), `dashed` (`setLineDash`+`lineDashOffset`) — over the already-wired lyon cap/join/dash fields.
- **Styling & layout:** `aspect_ratio`/`aspect_square`/`aspect_video`; **real CSS Grid track sizing** (`GridTrack` px/rem/fr/auto/min-content/max-content/fraction/minmax/repeat + `grid_template_columns`/`grid_template_rows`/`grid_auto_flow`, mapped to Taffy 0.9) plus **grid inline-axis alignment** (`justify_items`/`justify_self` + `justify_items_{start,center,end,stretch}` / `justify_self_*` conveniences — the missing CSS `justify-items`/`justify-self`, mapped to Taffy and unit-tested; grid item placement `col_*`/`row_*` was already present); `glow`/`shadow_inner` presets; `refine_style` reusable style presets; and semantic `success`/`warning` theme tokens (the previously off-token hardcoded greens/ambers now route through `ThemeTokens` across **Alert/Progress/AnimatedProgress/Timeline/NotificationCenter/CopyButton/PulseIndicator** — a full-codebase audit, so custom themes control every status color).
- **Component freedom (priority #2):** open `Custom(Colors)` variants + `.colors()`/`.color()` for Button, IconButton, Input, Textarea, Alert, Progress, CircularProgress, AnimatedProgress, and Spinner; reusable style presets (`Styled::refine_style`); and the **start of the headless primitive layer** (the report's #1 anti-sameness lever) — `kael_ui::headless` with 13 unstyled-but-correct state machines (Disclosure, Toggle, Select, Tabs, Slider, Combobox, Accordion, Pagination, Stepper, RadioGroup, ToastQueue, Carousel, Tree), so apps can render any visual over correct interaction logic instead of forking styled components.
...
- **DX / scaffolder (front door):** `kael new` is now self-contained — template sources are embedded at compile time (`include_str!`) instead of read from the repo's `templates/` dir, so the scaffolder works as a standalone installed binary. Verified end-to-end (`cargo test -p xtask -- --ignored`): it scaffolds a project and the generated app passes a nested `cargo check` against the workspace. Only the crates.io publish of the binary remains gated.
- **Rendering is pixel-verifiable here (this invalidates a whole "gated" category):** the repo already ships a golden-image harness — `HeadlessRenderer` rasterizes an arbitrary `Scene` off-screen on the live Metal backend and reads back BGRA pixels; `golden::compare` does tolerant pixel-diffing. Added `render_scene_to_bytes` + a **golden-image GPU test suite** that pixel-proves the renderer: solid fills are uniform and correct, and **linear / radial / conic gradients each rasterize a true interpolated color range** (not a solid fill). So "shader work is not pixel-verifiable on this machine" was **wrong** — the prerequisite the analysis named for every shader item exists in-repo and runs green here.
- **Gradient text (already shipped; now multi-stop):** `kael_ui::GradientText` already renders per-character gradient text (`start_color`/`end_color`, sampled across the glyphs) — it was listed as an unimplemented shader gap by mistake. Extended it with `.colors([..])` for **3+ stops** (rainbow/multi-color headings) via an evenly-sampled multi-stop CPU lerp; unit-tested.
- **Multi-stop gradients up to 8 stops (priority #2/#3 — the "gradients capped at 4 stops" gap, BRIDGED + pixel-verified):** the GPU pipeline now carries 8 stops (was 4). Reworked gradient color evaluation to read the stop colors **fragment-side from the uniform stop array in a loop** (instead of pre-computing 4 colors per-vertex and threading them through `color0..color3` varyings), removing the varying-count limit. Done in **both** shader backends — the hand-written Metal `shaders.metal` (the macOS path, pixel-verified here) and the `blade/shaders.wgsl` (Linux path, mirrored). `Background.colors` is now `[LinearColorStop; 8]`; the Rust↔shader struct layout is guarded by the existing naga layout test, and a golden test renders an 8-stop gradient whose **6th stop (green) only appears if the pipeline carries >4 stops** — it does. Linear/radial/conic/solid golden tests all still pass (no regression).
- **All six blend modes read the backdrop (the "blend modes never sample backdrop" correctness bug, BRIDGED + pixel-verified on Metal):** the old shader applied `src×src` / `src` vs a constant `0.5` — never the real backdrop. Now:
  - **Multiply + Screen** use **dedicated fixed-function blend pipelines** (`DestinationColor`/`Zero` and `One`/`OneMinusSourceColor`) — no framebuffer fetch, no MSAA/portability cost. Pixel-verified: white-Multiply leaves a gray backdrop unchanged (real `src×dst`), black-Screen likewise.
  - **Overlay + SoftLight + Difference** (non-linear in the destination) use a **framebuffer-fetch fragment** (`quad_fragment_blend`, reading `[[color(0)]]`) with the W3C formulas, run on a dedicated pipeline (blending disabled, shader composites). This needed `-mmacosx-version-min=11.0` to compile (an earlier batch wrongly concluded it was unblockable — it was just the version-min flag); GPUs without programmable blending fall back via a runtime `Option`. Pixel-verified: Difference(white,white)→black and Overlay(gray,white)→white, both of which the old approximation got wrong. Applies to simple (unbordered/unrounded) quads; bordered/rounded blend quads keep the approximation.
  - Routed via a per-blend-mode **run-split** in `draw_quads` — the common all-normal run draws in one call, **no regression** (every solid/gradient golden test passes through it). Metal path; blade/directx keep their current approximation (no regression).

### Not bridged — verified blockers and what each needs to unblock

| Gap (report ref) | Verified blocker | Prerequisite to unblock |
|---|---|---|
| Intrinsic sizing (`min/max/fit-content` width/height) | **Verified empirically:** `CompactLength` *has* `min_content()`/`max_content()`/`fit_content_px()`, and `Dimension::from_raw` can wrap them — but they're only valid for grid *tracks*. A test that put `max-content` on a node's width **panics** in taffy `util/resolve.rs:68` (`Dimension::maybe_resolve` hits `_ => unreachable!()` for content variants). So width/height content-sizing is genuinely unsupported in 0.9. | Taffy version that handles content variants in `Dimension::maybe_resolve`. |
| `calc()` lengths | **Verified empirically:** taffy 0.9 *does* have `Dimension::calc(ptr)` + a `calc` feature, and `maybe_resolve` handles it — BUT the built-in `TaffyTree` (the high-level API kael uses) stubs `resolve_calc_value` to return `0.0`, and taffy's own source comments `// TODO: implement calc() in high-level API`. Using it would require a full custom `LayoutPartialTree` implementation (reimplementing tree storage + every trait method). | Taffy implementing calc in `TaffyTree`, or a custom layout tree. |
| GPU atlas byte-budget LRU eviction (#1/#2) | **Re-verified, corrected:** atlas removal is **ref-counted and already wired** (`cache.rs` `Drop`→`remove`, `window.rs` sprite removal), so eviction is *re-rasterization, not corruption* — my earlier "corruption risk" was overstated. The genuine remainder is byte-budget LRU eviction of long-lived **glyph** tiles, which lives in the per-backend atlas (`metal`/`blade`/`directx`) — only `metal`+`test` are verifiable on macOS; `blade`/`directx` are compile-only-or-less here. Manager (`GpuMemoryManager`) + budget query are landed, tested. | On-device runs across the 3 GPU backends to confirm glyph re-rasterization after eviction. |
| Clip-path/mask, wide-gamut, dual-Kawase blur | **All six blend modes are now bridged** (see shipped list) — the earlier "framebuffer fetch is build-blocked" claim was *wrong*: it compiles fine, the build was just targeting `-mmacosx-version-min=10.15.7`; raising it to `11.0` (Big Sur; Catalina is EOL) unblocks programmable blending, with a runtime `Option` fallback for GPUs without it. Remaining: clip-path/mask needs a stencil/SDF mask path; wide-gamut isn't verifiable via the 8-bit readback (and the pipeline is already extended-range-capable internally); dual-Kawase only optimizes the working Gaussian. | Clip: stencil/SDF subsystem. Wide-gamut: a P3 display to verify. Kawase: low value. |
...
| `kael-cli` (`cargo install` + `kael new`) | **Template embedding is landed** — the scaffolder now embeds each template's source at compile time (`include_str!`) and no longer reads the repo's `templates/` dir, so it works as a standalone binary (verified end-to-end: scaffold + nested `cargo check` of the generated app). Only the crates.io publish itself stays gated. | crates.io access. |
| Headless-component layer (full) | **Re-verified, corrected:** the styled components are **controlled/stateless** `RenderOnce` (the parent owns state — e.g. `Toggle` takes `checked: bool`), so they do **not** duplicate the controllers' state machines; the controllers serve *uncontrolled* usage. "Reskin = move state into a controller" therefore does not apply — the layer is genuinely complete with the 13 tested controllers, not a partial deliverable with reskin debt. | None — this gap is bridged, not deferred. |
| Damage/dirty-region rendering, incremental layout, code hot-reload | XL multi-week rewrites. **Partial coverage already exists:** view-level dirty tracking (`dirty`/`dirty_views` gate re-render) and a now **content-complete frame checksum** (`Scene::structural_checksum` hashes the geometry + color of **every** primitive type — quads incl. border color + blend mode, shadows, blur rects, paths, underlines, mono + poly sprites — not just quad geometry, so it won't collide on a border/shadow/image change; the frame-identity primitive a safe skip-render needs, unit-tested incl. border-color detection). The **coarse whole-frame early-out is now LANDED end-to-end** (`Window::set_frame_skip_enabled`, opt-in/default-off): `Window::present` skips the GPU rasterize + present when the scene's content checksum matches the last presented frame (the compositor retains the prior contents — standard CAMetalLayer/swapchain behavior). Guards: frames with live external surfaces (video) are never skipped (`Scene::has_live_surfaces`), a resize invalidates the tracker (`bounds_changed`), and the checksum now also hashes **transforms** (quad + glyph) so transform-only animations don't freeze. The decision logic + checksum (incl. transform and border-color detection) are unit-tested; the present-loop skip itself is opt-in/default-off because the "compositor keeps the last drawable" path can't be exercised in a headless test (standard behavior, but unverified-in-CI). **Fine-grained dirty-*region* (per-rect GPU updates) is now substantially bridged too:** `Scene::damage_since(prev) -> FrameDamage` computes a conservative damage rectangle by comparing per-primitive content fingerprints at matching draw-order indices (unioning the vacated + newly-occupied bounds of any changed primitive; falling back to `Full` on a primitive-count change or a live surface) — unit-tested for the None / single-rect / moved-quad-union / structural-Full cases. The **GPU scissor + `Load` partial-repaint mechanism** it feeds is implemented (`MetalRenderer::render_damage_to_bytes`: render base, then re-rasterize only the damage rect of the next frame with `MTLScissorRect` + `MTLLoadAction::Load`) and **golden-verified on real Metal** — a per-rectangle partial repaint is pixel-identical (≤2/255 max channel diff) to a full repaint (`per_rectangle_partial_repaint_matches_a_full_repaint`). This disproves the earlier "can't verify headlessly" assumption. The only remaining piece is wiring `damage_since` into the *live* window present loop, which is opt-in/unverified-in-CI for the same swapchain reason as the whole-frame skip (and is a correctness hazard to enable blindly: `next_drawable()` rotates 2–3 buffers, so a live `Load` reloads a several-frames-stale drawable unless drawable retention is managed). Full incremental relayout (**verified XL:** `window.rs:2289` calls `layout_engine.clear()` at the end of every draw, so the taffy tree is rebuilt from scratch each frame — retaining it for incrementality needs stable element→node IDs across frames, i.e. retained-mode diffing) remains. **Hot reload is now partly bridged:** *style/token* hot-reload SHIPS (`kael::dev::watch_styles(cx, path)` live-reloads color/typography/spacing/radii/shadow tokens from a JSON/TOML file onto the global `Theme`, on top of the already-wired `App::observe_theme_file`; unit-tested) — only *code* hot-patch (`subsecond`/dylib) is the XL remainder; shaders already hot-reload via `runtime_shaders`. | Per-rect computation + GPU mechanism done & verified; style hot-reload shipped; live wiring + incremental layout + code hot-patch remain. |
