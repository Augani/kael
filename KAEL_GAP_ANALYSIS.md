...
| Multi-stop gradients (>4 stops) | **Re-verified with code evidence:** the Rust builders no longer silently truncate (now resample to 4 stops preserving endpoints — landed). Going beyond 4 stops is gated by the **vertex→fragment varying interface**: prepared stop colors flow as `QuadVarying`/`PathVarying` `color0..color3` (computed in `vs_quad`, interpolated to `fs_quad`), so >4 needs more varyings — a hardware-limited resource, unverifiable here. | A golden-image GPU harness + a varying-budget audit per backend. |
| Blend-mode dst-read, clip-path/mask, wide-gamut, gradient text, dual-Kawase blur | Shader work; not pixel-verifiable on this machine. | A golden-image GPU test harness (per-backend tolerance). |
