...
| Low | Glyph/shadow atlas never evicts | GPU memory climbs for hours | **Done end-to-end on Metal + app-facing, on-device-verified.** Frame-protected LRU byte-budget eviction wired through the whole stack: `App::set_atlas_byte_budget(Some(bytes))` → `Window` → `PlatformWindow` (no-op default for other backends) → `MetalRenderer` → `MetalAtlas` (per-tile `last_used`, etagere region reclaim), evicting to budget at end-of-frame while protecting the swapchain in-flight window. Default `None` = unchanged. 6 on-device atlas tests (LRU order, current-frame + keep-window protection, region reuse). **Remaining:** replicate on blade/directx (compile-only here). | S (Metal end-to-end + app API; other backends pending) |
