...
| Low | Glyph/shadow atlas never evicts | GPU memory climbs for hours | **Done end-to-end on Metal (opt-in), on-device-verified.** `select_atlas_evictions` (LRU byte-budget, frame-protected) is wired into `MetalAtlas` (per-tile `last_used`, `advance_frame`, `evict_to_budget*` reclaiming each tile's etagere region) AND into the render loop: `MetalRenderer::set_atlas_byte_budget(Some(bytes))` makes `draw` evict to budget + advance the atlas clock at end-of-frame, protecting the swapchain's in-flight window (`evict_to_budget_keeping(budget, 3)`) so no in-flight tile is reclaimed. Default `None` = unchanged behavior. 6 on-device tests incl. the keep-window protection. **Remaining:** expose via `App::set_gpu_budget` plumbing; replicate on blade/directx (compile-only here). | S (Metal end-to-end; App wiring + other backends pending) |
