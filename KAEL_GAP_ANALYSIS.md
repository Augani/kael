...
| Damage/dirty-region rendering, incremental layout, code hot-reload | XL multi-week rewrites. **Partial coverage already exists:** view-level dirty tracking (`dirty`/`dirty_views` gate re-render) and a now **content-complete frame checksum** (`Scene::structural_checksum` hashes the geometry + color of **every** primitive type — quads incl. border color + blend mode, shadows, blur rects, paths, underlines, mono + poly sprites — not just quad geometry, so it won't collide on a border/shadow/image change; the frame-identity primitive a safe skip-render needs, unit-tested incl. border-color detection). The **coarse whole-frame early-out logic is now landed + verified** too — `FrameSkip` (`kael::FrameSkip`, opt-in): records the per-frame content checksum and reports whether a frame is identical to the last presented one (so it can be skipped, since the compositor retains the prior contents). Unit-tested incl. enable/change/invalidate and a real-`Scene`-checksum-driven case. The remaining step for it is the present-loop integration (a guarded `should_skip` check that skips the GPU submit) — left unwired because whether the compositor retains the last drawable on a skipped present can't be verified in a headless test. The XL remainder is fine-grained dirty-*region* (per-rect GPU updates); full incremental relayout (**verified XL:** `window.rs:2289` calls `layout_engine.clear()` at the end of every draw, so the taffy tree is rebuilt from scratch each frame — retaining it for incrementality needs stable element→node IDs across frames, i.e. retained-mode diffing); and code (not shader — `runtime_shaders` already hot-reloads shaders) hot-reload. | Dedicated multi-week workstreams. |
