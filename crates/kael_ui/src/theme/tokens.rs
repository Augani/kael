...
impl ThemeTokens {
    pub fn gradient_primary(&self) -> kael::Background {
        let c = self.primary;
        let darkened = hsla(c.h, c.s, (c.l - 0.08).max(0.0), c.a);
        kael::linear_gradient(
            180.0,
            kael::linear_color_stop(c, 0.0),
            kael::linear_color_stop(darkened, 1.0),
        )
    }

    pub fn gradient_surface(&self) -> kael::Background {
        let c = self.card;
        let lightened = hsla(c.h, c.s, (c.l + 0.02).min(1.0), c.a);
        kael::linear_gradient(
            135.0,
            kael::linear_color_stop(c, 0.0),
            kael::linear_color_stop(lightened, 1.0),
        )
    }

    pub fn gradient_accent(&self) -> kael::Background {
        let c = self.accent;
    ...
    pub fn gradient_destructive(&self) -> kael::Background {
    ...
    pub fn layered_gradient(&self, angle: f32, colors: &[Hsla]) -> Vec<kael::Background> {
        if colors.len() < 2 {
            return vec![];
        }
        ...
        for window in colors.windows(2) {
            layers.push(kael::linear_gradient(
                angle,
                kael::linear_color_stop(window[0], 0.0),
                kael::linear_color_stop(window[1], 1.0),
            ));
        }
        layers
    }
