...
use kael::{Menu, MenuItem, SharedString, StyleRefinement, Styled, SystemMenuType};
...
impl AppMenu {
    ...
    pub fn action<A: kael::Action>(mut self, label: impl Into<SharedString>, action: A) -> Self {
...
pub struct StandardMacMenuBar {
    // Kept for API compatibility; macOS sources the application menu title
    // from the bundle, so the name is not read on this path.
    #[allow(dead_code)]
