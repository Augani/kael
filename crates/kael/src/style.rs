...
pub struct Style {
    ...
    /// The saturation multiplier applied to blurred backdrop content.
    ...
    pub fn paint(
            ...
            && backdrop_blur > Pixels::ZERO
        {
            window.paint_blur(
                ...
                self.backdrop_saturate.unwrap_or(1.0),
            );
        }
...
pub enum Overflow {
    ...
    /// The automatic minimum size of this node as a flexbox/grid item should be `0`.
    /// Content that overflows this node should *not* contribute to the scroll region of its parent.
    /// Unlike [`Overflow::Scroll`], no scrollbar space is reserved during layout, and scrolling is
    /// only activated on axes whose content actually overflows the container.
    Auto,
...
mod tests {
    use crate::{
        BoxShadow, Context, IntoElement, ParentElement, Render, ScaledPixels, TestAppContext,
        Window, blue, div, green, hsla, point, px, red, yellow,
    };
    ...
    #[kael::test]
    fn styled_backdrop_blur_emits_a_blur_rect(cx: &mut TestAppContext) {
        let (_view, cx) = cx.add_window_view(|_, _| BackdropBlurTestView);

        cx.update(|window, _| {
            assert_eq!(window.rendered_scene().blur_rects.len(), 1);

            let blur_rect = &window.rendered_scene().blur_rects[0];
            assert_eq!(blur_rect.blur_radius, ScaledPixels(24.0));
            assert_eq!(blur_rect.bounds.size.width, ScaledPixels(80.0));
    ...
    #[kael::test]
    fn styled_shadow_renders_as_gpu_primitive(cx: &mut TestAppContext) {
        ...
        cx.update(|window, _| {
            ...
            assert!(shadow.blur_radius.0 > 0.0);
            assert!(!shadow.bounds.is_empty());
        });
    }
}
