...
impl DirectWriteState {
    ...
    fn layout_line_with_features(
        ...
        if features.is_empty() {
            return self.layout_line(text, font_size, font_runs);
        }
        if font_runs.is_empty() {
            return Ok(LineLayout {
                font_size,
        ...
        unsafe {
            ...
            // set it on the full layout range. Unsupported tags are silently
            // ignored per Req 29.4 — DirectWrite will simply skip unknown tags.
            let extra_typography = self.components.factory.CreateTypography()?;
            for feature in features {
                let tag_str = match std::str::from_utf8(&feature.tag) {
                    Ok(s) if s.len() == 4 => s,
                    _ => continue, // silently ignore invalid tags
                };
                let dw_feature = make_direct_write_feature(tag_str, feature.value);
                // AddFontFeature may fail for truly malformed data; ignore errors
                // to satisfy the "ignore unsupported tags without error" requirement.
                let _ = extra_typography.AddFontFeature(dw_feature);
            }
    ...
    fn create_glyph_run_analysis(
        ...
        let antialias_mode = if params.raster_mode == GlyphRasterMode::Subpixel {
    ...
    fn raster_bounds(&self, params: &RenderGlyphParams) -> Result<Bounds<DevicePixels>> {
        ...
        let texture_type = if params.raster_mode == GlyphRasterMode::Subpixel {
    ...
    fn rasterize_glyph(
        ...
        let bitmap_data = if params.is_emoji {
            ...
            self.rasterize_subpixel(params, glyph_bounds)?
        } else {
    ...
    fn rasterize_subpixel(
        ...
        let mut cleartype_data = vec![0u8; pixel_count * 3];
        ...
        Ok(pack_cleartype_bgra(&cleartype_data, pixel_count))
    }
...
impl IDWriteTextRenderer_Impl for TextRenderer_Impl {
    ...
    fn DrawUnderline(
        ...
        // GPUI handles underline rendering at the scene level via paint_underline
        // in line.rs, so this callback just needs to succeed without error.
        // This allows IDWriteTextLayout::SetUnderline to be called on the layout
        // without causing Draw() to fail.
        Ok(())
    }
    ...
    fn DrawStrikethrough(
        ...
        // GPUI handles strikethrough rendering at the scene level via
        // paint_strikethrough in line.rs, so this callback just needs to succeed
        // without error. This allows IDWriteTextLayout::SetStrikethrough to be
        // called on the layout without causing Draw() to fail.
        Ok(())
    }
...
fn pack_cleartype_bgra(cleartype_data: &[u8], pixel_count: usize) -> Vec<u8> {
    let mut bgra = vec![0u8; pixel_count * 4];
    for index in 0..pixel_count {
        let src = index * 3;
        let r = cleartype_data.get(src).copied().unwrap_or(0);
        let g = cleartype_data.get(src + 1).copied().unwrap_or(0);
        let b = cleartype_data.get(src + 2).copied().unwrap_or(0);
        let dst = index * 4;
        bgra[dst] = b;
        bgra[dst + 1] = g;
        bgra[dst + 2] = r;
        bgra[dst + 3] = r.max(g).max(b);
    }
    bgra
}
...
#[cfg(test)]
mod tests {
    use crate::platform::windows::direct_write::{ClusterAnalyzer, pack_cleartype_bgra};
    ...
    #[test]
    fn cleartype_packs_rgb_into_bgra_with_dominant_alpha() {
        let cleartype = [10u8, 20, 30, 200, 5, 5];
        let bgra = pack_cleartype_bgra(&cleartype, 2);
        assert_eq!(bgra, vec![30, 20, 10, 30, 5, 5, 200, 200]);
    }

    #[test]
    fn cleartype_packs_zero_coverage_as_transparent() {
        let cleartype = [0u8, 0, 0];
        let bgra = pack_cleartype_bgra(&cleartype, 1);
        assert_eq!(bgra, vec![0, 0, 0, 0]);
    }

    #[test]
    fn cleartype_tolerates_truncated_input() {
        let cleartype = [255u8];
        let bgra = pack_cleartype_bgra(&cleartype, 1);
