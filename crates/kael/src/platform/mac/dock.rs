...
pub fn request_user_attention(attention_type: AttentionType) -> isize {
    let Some(mtm) = MainThreadMarker::new() else {
        log::error!("request_user_attention ignored off the macOS main thread");
        return 0;
    };
...
pub fn cancel_user_attention(request_id: isize) {
    let Some(mtm) = MainThreadMarker::new() else {
        log::error!("cancel_user_attention ignored off the macOS main thread");
        return;
    };
    let app = NSApplication::sharedApplication(mtm);
...
pub fn set_dock_badge(label: Option<&str>) {
    let Some(mtm) = MainThreadMarker::new() else {
        log::error!("set_dock_badge ignored off the macOS main thread");
        return;
    };
    let app = NSApplication::sharedApplication(mtm);
