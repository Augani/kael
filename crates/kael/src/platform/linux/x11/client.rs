...
use x11rb::{
    ...
    protocol::{Event, present, randr, render, xinput, xkb, xproto},
...
pub struct X11ClientState {
    ...
    /// Whether the X11 Present extension is available for VSync fallback
    #[allow(dead_code)]
...
impl X11ClientStatePtr {
    ...
    pub fn set_frame_polling(&self, x_window: u32, active: bool) {
        let Some(client) = self.get_client() else {
            return;
        };
        ...
        window_ref.frame_polling_active = active;
...
impl X11Client {
    pub(crate) fn new() -> anyhow::Result<Self> {
            ...
            .insert_source(network_rx, {
                move |event, _, client: &mut X11Client| {
                    if let calloop::channel::Event::Msg(status) = event {
                        client.handle_network_status_change(status);
                    }
                }
            })
            .map_err(|err| anyhow!("Failed to initialize network status event source: {err:?}"))?;

        handle
            .insert_source(system_power_rx, {
                move |event, _, client: &mut X11Client| {
                    if let calloop::channel::Event::Msg(power_event) = event {
                        client.handle_system_power_event(power_event);
                    }
                }
            })
        ...
        // Check if the X11 Present extension is available for VSync fallback
        let present_supported = xcb_connection
            .extension_information(present::X11_EXTENSION_NAME)
            .ok()
            .flatten()
            .is_some();
        log::info!("x11: Present extension supported: {}", present_supported);

        // Subscribe to randr screen change notifications so we can update
        // refresh rates when monitors change
        let _ = xcb_connection.randr_select_input(
            root,
    ...
    /// Handle a network status change event received from the NetworkManager D-Bus monitor.
    fn handle_network_status_change(&self, status: crate::NetworkStatus) {
        let mut state = self.0.borrow_mut();
    ...
    fn handle_system_power_event(&self, event: crate::SystemPowerEvent) {
        let mut state = self.0.borrow_mut();
    ...
    fn handle_event(&self, event: Event) -> Option<()> {
        match event {
            ...
            Event::KeyPress(event) => {
                // Check for global hotkey match before trying to resolve the window,
                // since grabbed keys on the root window won't have an app window.
                {
                    let mut state = self.0.borrow_mut();
                    if let Some(hotkey_id) = state
                        .global_hotkey
                        .find_hotkey_for_key(event.detail, event.state.into())
                    {
                ...
                // Check for media key events before resolving the window,
                // so media keys are dispatched even when no window has focus.
                {
                    let mut state = self.0.borrow_mut();
            ...
            Event::KeyRelease(event) => {
                // Check for global hotkey key-up before trying to resolve the window,
                // since grabbed keys on the root window won't have an app window.
                {
                    let mut state = self.0.borrow_mut();
                    let released_ids = state.global_hotkey.find_pressed_for_keycode(event.detail);
            ...
            Event::RandrScreenChangeNotify(_) | Event::RandrNotify(_) => {
                // Screen configuration changed (e.g., monitor switch, resolution change).
                // Re-query refresh rates and restart refresh loops for all visible windows.
                let mut state = self.0.borrow_mut();
                let window_ids: Vec<xproto::Window> = state.windows.keys().copied().collect();
                for x_window in window_ids {
                    if let Some(window_ref) = state.windows.get_mut(&x_window) {
                        // Reset refresh state so it gets re-queried with the new rate
                        ...
                        // refresh_state is now None, so update_refresh_loop will re-query
                    }
                    state.update_refresh_loop(x_window);
                }
            }
...
impl LinuxClient for X11Client {
    ...
    fn open_window(
        ...
        let parent_window = params
            .parent
            .map(|parent_handle| {
                let parent = find_x_window_for_handle(&state, Some(parent_handle));
                if parent.is_none() {
                    log::warn!(
                        "X11: unable to resolve explicit parent window {:?}",
                        parent_handle
                    );
                }
                parent
            ...
            .unwrap_or_else(|| {
                if params.kind == crate::WindowKind::Floating {
                    find_x_window_for_handle(&state, None)
                } else {
                    None
                }
            });
    ...
    fn system_idle_time(&self) -> Option<Duration> {
        ...
        // Try X11 screensaver extension first (most accurate on X11)
        if let Some(reply) =
            x11rb::protocol::screensaver::query_info(&*state.xcb_connection, screen.root)
                .ok()
                .and_then(|cookie| cookie.reply().ok())
        {
            return Some(Duration::from_millis(reply.ms_since_user_input as u64));
        }
        // Fall back to D-Bus ScreenSaver.GetSessionIdleTime
...
impl X11ClientState {
    ...
    fn update_refresh_loop(&mut self, x_window: xproto::Window) {
        ...
        let should_refresh = window_ref.frame_polling_active
            && window_ref.is_mapped
            && !matches!(window_ref.last_visibility, Visibility::FULLY_OBSCURED);
        match (should_refresh, window_ref.refresh_state.take()) {
