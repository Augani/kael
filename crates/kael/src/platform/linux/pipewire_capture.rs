...
impl DeviceEnumerator for PipeWireCaptureBackend {
...
impl CaptureBackend for PipeWireCaptureBackend {
    fn create_session(&self, config: &CaptureConfig) -> Result<Box<dyn CaptureSession>> {
        match config.kind {
            CaptureDeviceKind::Screen | CaptureDeviceKind::Camera => {
                Ok(Box::new(PipeWireCaptureSession::new(config.clone())))
            }
            _ => Err(anyhow!(
                "PipeWireCaptureBackend does not support {:?}",
                config.kind
            )),
        }
    }
