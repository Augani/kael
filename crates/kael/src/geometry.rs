...
impl Point<Pixels> {
    ...
    ///
    /// ```
    /// # use kael::{Point, Pixels, ScaledPixels};
...
impl Size<Pixels> {
    ...
    ///
    /// ```
    /// # use kael::{Size, Pixels, ScaledPixels};
    ...
    T: Add<T, Output = T> + Clone + Debug + Default + PartialEq,
{
    ...
    ///
    /// ```
    /// use kael::{Bounds, Corner, Point, Size};
    /// let bounds = Bounds {
    ///     origin: Point { x: 0, y: 0 },
    ///     size: Size { width: 10, height: 20 },
    /// };
...
impl Bounds<Pixels> {
    ...
    ///
    /// ```
    /// # use kael::{Bounds, Point, Size, Pixels, ScaledPixels, DevicePixels};
    /// let bounds = Bounds {
    ...
    /// Scale bounds to device coordinates and snap to the device pixel grid.
    ///
    /// The origin and far edge are rounded independently so adjacent elements
    /// share exact pixel boundaries without gaps or overlaps.
    pub fn scale_and_snap(&self, factor: f32) -> Bounds<ScaledPixels> {
        ...
        let snapped_origin_x = scaled_origin_x.round();
        let snapped_origin_y = scaled_origin_y.round();
        let snapped_far_x = scaled_far_x.round();
        let snapped_far_y = scaled_far_y.round();
...
impl Edges<Length> {
    ...
    ///
    /// ```
    /// # use kael::{DefiniteLength, Edges, Length, Pixels};
...
impl Edges<DefiniteLength> {
    ...
    ///
    /// ```
    /// # use kael::{px, DefiniteLength, Edges};
    ...
    ///
    /// ```
    /// # use kael::{Edges, DefiniteLength, px, AbsoluteLength, rems, Size};
    /// let edges = Edges {
...
impl Edges<AbsoluteLength> {
    ...
    ///
    /// ```
    /// # use kael::{AbsoluteLength, Edges, Pixels};
    ...
    ///
    /// ```
    /// # use kael::{Edges, AbsoluteLength, Pixels, px, rems};
    /// let edges = Edges {
...
impl Edges<Pixels> {
    ...
    ///
    /// ```
    /// # use kael::{Edges, Pixels, ScaledPixels};
    /// let edges = Edges {
    ...
    /// Scale edges and round each to the nearest device pixel.
    pub fn scale_and_snap(&self, factor: f32) -> Edges<ScaledPixels> {
        Edges {
            top: ScaledPixels((self.top.0 * factor).round()),
...
impl Corners<AbsoluteLength> {
    ...
    ///
    /// ```
    /// # use kael::{Corners, AbsoluteLength, Pixels, Rems, Size};
    /// let corners = Corners {
...
impl Corners<Pixels> {
    ...
    ///
    /// ```
    /// # use kael::{Corners, Pixels, ScaledPixels};
    /// let corners = Corners {
...
impl AbsoluteLength {
    ...
    ///
    /// ```
    /// # use kael::{AbsoluteLength, Pixels, Rems};
...
impl DefiniteLength {
    ...
    ///
    /// ```
    /// # use kael::{DefiniteLength, AbsoluteLength, Pixels, px, rems};
