...
impl PathBuilder {
    ...
    /// Sets the dash offset for a stroked path.
    pub fn dash_offset(mut self, dash_offset: Pixels) -> Self {
    ...
    pub(crate) fn stroke_source_path(
        path: &lyon::path::Path,
        options: StrokeOptions,
        dash_array: Option<Vec<Pixels>>,
        dash_offset: Pixels,
    ) -> Result<Path<Pixels>, Error> {
        Self::tessellate_stroke(dash_array, dash_offset, path, &options)
    }
    ...
    fn tessellate_fill(
        ...
        Ok(Self::build_path_with_source(
            buf,
    ...
    fn tessellate_stroke(
        ...
        let path = if let Some(dash_array) = dash_array {
            ...
            let total_dash_length: f32 = dash_array.iter().map(|segment| segment.0.max(0.0)).sum();
            if total_length == 0.0 || total_dash_length == 0.0 {
                return Ok(Path::new(Point::default()));
            }

            let mut dash_index = 0;
            let mut dash_position = dash_offset.0.rem_euclid(total_dash_length);
            while dash_position >= dash_array[dash_index % dash_array_len].0 {
                dash_position -= dash_array[dash_index % dash_array_len].0;
                dash_index += 1;
            }

            let mut pos = 0.;
            while pos < total_length {
                let dash_length = dash_array[dash_index % dash_array_len].0 - dash_position;
    ...
    pub fn build_path(buf: VertexBuffers<lyon::math::Point, u16>) -> Path<Pixels> {
        Self::build_path_with_source(buf, None)
    }

    fn build_path_with_source(
        buf: VertexBuffers<lyon::math::Point, u16>,
        source_path: Option<Arc<lyon::path::Path>>,
    ) -> Path<Pixels> {
        ...
        if let Some(source_path) = source_path {
            path = path.with_source_path(source_path);
        }
