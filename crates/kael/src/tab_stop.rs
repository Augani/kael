...
impl TabStopMap {
    ...
    pub fn next_in_group(&self, focused_id: Option<&FocusId>) -> Option<FocusHandle> {
        let focused_id = focused_id?;
        let node = self.tab_node_for_focus_id(focused_id)?;
        let group_path = self.group_path(node)?;
        self.next_in_group_inner(node, &group_path)
            .or_else(|| self.first_in_group(&group_path))
            .and_then(|order| self.focus_handle_for_order(order))
    }

    pub fn prev_in_group(&self, focused_id: Option<&FocusId>) -> Option<FocusHandle> {
        let focused_id = focused_id?;
        let node = self.tab_node_for_focus_id(focused_id)?;
        let group_path = self.group_path(node)?;
        self.prev_in_group_inner(node, &group_path)
            .or_else(|| self.last_in_group(&group_path))
            .and_then(|order| self.focus_handle_for_order(order))
    }

    fn next_in_group_inner(
        &self,
        node: &TabStopNode,
        group_path: &TabStopPath,
    ) -> Option<&TabStopNode> {
        let mut cursor = self.order.cursor::<TabStopNode>(());
        cursor.seek(node, Bias::Left);
        cursor.next();
        while let Some(item) = cursor.item() {
            if !self.path_is_in_group(&item.path, group_path) {
                return None;
            }
            if item.tab_stop {
                return Some(item);
    ...
    fn first_in_group(&self, group_path: &TabStopPath) -> Option<&TabStopNode> {
        let first = self.order.first()?;
        let mut cursor = self.order.cursor::<TabStopNode>(());
        cursor.seek(first, Bias::Left);
        while let Some(item) = cursor.item() {
            if self.path_is_in_group(&item.path, group_path) && item.tab_stop {
                return Some(item);
            }
            cursor.next();
        }
    ...
    fn last_in_group(&self, group_path: &TabStopPath) -> Option<&TabStopNode> {
        let last = self.order.last()?;
        let mut cursor = self.order.cursor::<TabStopNode>(());
        cursor.seek(last, Bias::Left);
        while let Some(item) = cursor.item() {
            if self.path_is_in_group(&item.path, group_path) && item.tab_stop {
                return Some(item);
            }
    ...
    fn group_path(&self, node: &TabStopNode) -> Option<TabStopPath> {
        if node.path.0.len() <= 1 {
            return None;
        }

        let mut group_path = node.path.clone();
        group_path.0.pop();
        Some(group_path)
    }

    fn path_is_in_group(&self, path: &TabStopPath, group_path: &TabStopPath) -> bool {
        path.0.starts_with(group_path.0.as_slice())
    }
...
mod tests {
    ...
    #[test]
    fn test_group_local_traversal() {
        let focus_map = Arc::new(FocusMap::default());
        let mut tab_index_map = TabStopMap::default();

        let first = FocusHandle::new(&focus_map).tab_stop(true).tab_index(0);
        let second = FocusHandle::new(&focus_map).tab_stop(true).tab_index(1);
        let outside = FocusHandle::new(&focus_map).tab_stop(true).tab_index(2);

        tab_index_map.begin_group(0);
        tab_index_map.insert(&first);
        tab_index_map.insert(&second);
        tab_index_map.end_group();
        tab_index_map.insert(&outside);

        assert_eq!(
            tab_index_map
                .next_in_group(Some(&first.id))
                .map(|handle| handle.id),
            Some(second.id)
        );
        assert_eq!(
            tab_index_map
                .next_in_group(Some(&second.id))
                .map(|handle| handle.id),
            Some(first.id)
        );
        assert_eq!(
            tab_index_map
                .prev_in_group(Some(&first.id))
                .map(|handle| handle.id),
            Some(second.id)
        );
        assert_eq!(tab_index_map.next_in_group(Some(&outside.id)), None);
    }
