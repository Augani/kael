...
//! (In practice, you just need to add a new line to
//!  your keymaps configuration file).
//!
//! ```ignore
//! cx.bind_keys([
...
//! bubbles up from the bottom. For example, the Workspace is the top-level view, which contains Pane's, which contain Editors. If there are conflicting keybindings defined
    ...
    fn test_input_handler_pending(cx: &mut TestAppContext) {
        ...
        impl CustomElement {
            fn new(cx: &mut Context<Self>) -> Self {
                Self {
                    ...
                    text: Rc::default(),
                    marked_range: Rc::default(),
                }
            }

            fn resolve_replacement_range_utf16(
                &self,
                replacement_range: Option<Range<usize>>,
            ) -> Range<usize> {
                replacement_range
                    .or_else(|| self.marked_range.borrow().clone())
                    .unwrap_or_else(|| {
                        let len = self.text.borrow().encode_utf16().count();
                        len..len
                    })
            }
            ...
            fn utf16_range_to_utf8(text: &str, range: &Range<usize>) -> Range<usize> {
                Self::utf16_to_utf8_index(text, range.start)
                    ..Self::utf16_to_utf8_index(text, range.end)
            }
            ...
            fn replace_text_internal(
                ...
                let replacement_range_utf16 =
                    self.resolve_replacement_range_utf16(replacement_range);
                let mut text = self.text.borrow_mut();
                let replacement_range_utf8 =
                    Self::utf16_range_to_utf8(&text, &replacement_range_utf16);
                text.replace_range(replacement_range_utf8, new_text);
            }
        }
        ...
        impl InputHandler for CustomElement {
            ...
            fn marked_text_range(&mut self, _: &mut Window, _: &mut App) -> Option<Range<usize>> {
                self.marked_range.borrow().clone()
            }
            ...
            fn text_for_range(
                ...
                *adjusted_range = Some(range_utf16);
                Some(text[range_utf8].to_string())
            }
            ...
            fn replace_text_in_range(
                ...
                self.replace_text_internal(replacement_range, text);
            ...
            fn replace_and_mark_text_in_range(
                ...
                let replacement_range_utf16 =
                    self.resolve_replacement_range_utf16(replacement_range.clone());
                self.replace_text_internal(replacement_range, new_text);
                *self.marked_range.borrow_mut() = (!new_text.is_empty()).then_some(
                    replacement_range_utf16.start
                        ..replacement_range_utf16.start + new_text.encode_utf16().count(),
                );
            }
