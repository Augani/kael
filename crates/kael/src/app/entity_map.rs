...
impl EntityMap {
    ...
    pub fn reserve<T: 'static>(&self) -> Slot<T> {
        let id = self.ref_counts.write().slots.insert(EntitySlot {
            ref_count: 1.into(),
            generation: 0.into(),
        });
    ...
    pub fn generation(&self, entity_id: EntityId) -> Option<u64> {
        let ref_counts = self.ref_counts.read();
        ref_counts
            .slots
            .get(entity_id)
            .map(|slot| slot.generation.load(SeqCst))
    }

    pub fn bump_generation(&self, entity_id: EntityId) -> Option<u64> {
        let ref_counts = self.ref_counts.read();
        ref_counts
            .slots
            .get(entity_id)
    ...
    pub fn take_dropped(&mut self) -> Vec<(EntityId, Box<dyn Any>)> {
        ...
        dropped_entity_ids
            ...
            .filter_map(|entity_id| {
                let slot = ref_counts.slots.remove(entity_id).unwrap();
                debug_assert_eq!(
                    slot.ref_count.load(SeqCst),
                    0,
...
impl Drop for AnyEntity {
    fn drop(&mut self) {
        if let Some(entity_map) = self.entity_map.upgrade() {
            ...
            let prev_count = slot.ref_count.fetch_sub(1, SeqCst);
            assert_ne!(prev_count, 0, "Detected over-release of a entity.");
            if prev_count == 1 {
...
impl AnyWeakEntity {
    ...
    pub fn upgrade(&self) -> Option<AnyEntity> {
        ...
        let ref_count = &ref_counts.slots.get(self.entity_id)?.ref_count;
