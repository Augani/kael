...
use crate::{
    AccessibilityAttributes, App, ArenaBox, AvailableSpace, Bounds, Context, DispatchNodeId,
    ELEMENT_ARENA, ElementId, FocusHandle, InspectorElementId, LayoutId, Pixels, Point, Size,
    Style, Window, util::FluentBuilder,
};
...
pub trait IntoElement: Sized {
    ...
    fn into_boxed_any_element(self) -> AnyElement {
        self.into_element().into_boxed_any()
    }
}
    ...
    E::RequestLayoutState: 'static,
{
    ...
    fn accessibility_attributes(&self) -> Option<AccessibilityAttributes> {
        self.element.accessibility_attributes()
    }
}

enum AnyElementStorage {
    Arena(ArenaBox<dyn ElementObject>),
    Owned(Box<dyn ElementObject>),
}

/// A dynamically typed element that can be used to store any element type.
pub struct AnyElement(AnyElementStorage);
...
impl AnyElement {
        ...
        E::RequestLayoutState: Any,
    {
        ...
        AnyElement(AnyElementStorage::Arena(element))
    }

    pub(crate) fn new_boxed<E>(element: E) -> Self
    where
        E: 'static + Element,
        E::RequestLayoutState: Any,
    {
        AnyElement(AnyElementStorage::Owned(Box::new(Drawable::new(element))))
    }

    fn element_object_mut(&mut self) -> &mut dyn ElementObject {
        match &mut self.0 {
            AnyElementStorage::Arena(element) => &mut **element,
            AnyElementStorage::Owned(element) => &mut **element,
        }
    }

    fn element_object(&self) -> &dyn ElementObject {
        match &self.0 {
            AnyElementStorage::Arena(element) => &**element,
            AnyElementStorage::Owned(element) => &**element,
        }
    }
    ...
    pub(crate) fn reset_for_reuse(&mut self) {
        self.element_object_mut().reset_for_reuse();
    }

    pub(crate) fn supports_reuse(&self) -> bool {
        matches!(self.0, AnyElementStorage::Owned(_))
    }

    /// Request the layout ID of the element stored in this `AnyElement`.
    /// Used for laying out child elements in a parent element.
    pub fn request_layout(&mut self, window: &mut Window, cx: &mut App) -> LayoutId {
        self.element_object_mut().request_layout(window, cx)
    }
    ...
    pub fn prepaint(&mut self, window: &mut Window, cx: &mut App) -> Option<FocusHandle> {
        ...
        self.element_object_mut().prepaint(window, cx);
    ...
    pub fn paint(&mut self, window: &mut Window, cx: &mut App) {
        self.element_object_mut().paint(window, cx);
    }
...
impl Element for AnyElement {
    ...
    fn accessibility_attributes(&self) -> Option<AccessibilityAttributes> {
        self.element_object().accessibility_attributes()
    }
}
