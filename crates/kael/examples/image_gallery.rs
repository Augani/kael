...
use kael::{
    ...
    actions, div, hash, http_client::ReqwestClient, image_cache, img, prelude::*, px, rgb, size,
};
...
impl ImageCacheProvider for SimpleLruCacheProvider {
    fn provide(&mut self, window: &mut Window, cx: &mut App) -> kael::AnyImageCache {
        window
...
struct SimpleLruCache {
    max_items: usize,
    usages: Vec<u64>,
    cache: HashMap<u64, kael::ImageCacheItem>,
}
...
impl ImageCache for SimpleLruCache {
    fn load(
        &mut self,
        resource: &kael::Resource,
        window: &mut Window,
        cx: &mut App,
    ) -> Option<Result<Arc<kael::RenderImage>, kael::ImageCacheError>> {
        ...
        self.cache
            .insert(hash, kael::ImageCacheItem::Loading(task.clone()));
        self.usages.insert(0, hash);
